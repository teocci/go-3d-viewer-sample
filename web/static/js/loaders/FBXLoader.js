...
/**
 * THREE.Loader loads FBX file and generates Group representing FBX scene.
...
class GeometryParser {
    ...
    parseNormals(node) {
        ...
        const buffer = node.Normals.a
        let indexBuffer = []
        if (referenceType === 'IndexToDirect') {

            if ('NormalIndex' in node) {

                indexBuffer = node.NormalIndex.a

            } else if ('NormalsIndex' in node) {

                indexBuffer = node.NormalsIndex.a
    ...
    parseUVs(node) {
        ...
        const mappingType = node.MappingInformationType
        const referenceType = node.ReferenceInformationType
        const buffer = node.UV.a
        let indexBuffer = []
        if (referenceType === 'IndexToDirect') {

            indexBuffer = node.UVIndex.a
    ...
    parseVertexColors(node) {

        const mappingType = node.MappingInformationType
        const referenceType = node.ReferenceInformationType
        const buffer = node.Colors.a
        let indexBuffer = []
        if (referenceType === 'IndexToDirect') {

            indexBuffer = node.ColorIndex.a
